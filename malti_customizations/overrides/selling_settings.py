...
def validate_selling_price(self):
    ...
    action, role_allowed_to_override = frappe.get_cached_value(
				"Selling Settings", "None", ["custom_vsp_maintain_same_rate_action", "custom_vsp_role_to_override_stop_action"]
			)
    ...
    for item in self.items:
        ...
        if flt(item.base_net_rate) < flt(last_purchase_rate_in_sales_uom):
            if action == "Stop":
                ...
                show_warning_for_selling_price(item.idx, item.item_name, last_purchase_rate_in_sales_uom, "last purchase rate")
